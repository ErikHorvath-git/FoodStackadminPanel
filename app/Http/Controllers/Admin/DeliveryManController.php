...
class DeliveryManController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'identity_number' => [
                'required',
                'unique:delivery_men,identity_number', // ensures uniqueness
                'string',
            ],
            'email' => 'required|unique:delivery_men,email',
            'phone' => 'required|regex:/^([0-9\s\-\+\(\)]*)$/|min:9|max:20|unique:delivery_men,phone',
            'zone_id' => 'required',
            'earning' => 'required',
            'vehicle_id' => 'required',
            ...
            'identity_number.unique' => 'This identity number is already taken',
            'identity_number.string' => 'Identity number must be string.',
        ...
        $validator->after(function ($validator) use ($request, $page_data) {
            if ($page_data) {
                foreach ($page_data['data'] as $item) {
                    if ($item['field_type'] == 'file' && $item['is_required'] == 1) {
                        if (! isset($request->additional_documents[$item['input_data']]) || count($request->additional_documents[$item['input_data']]) < 1) {
                            $validator->errors()->add($item['input_data'], translate('messages.atleast_one_file_is_required_for_'.$item['input_data']));
                        }
                    }
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator->after(function ($validator) use ($request, $page_data, $additional_documents) {
            if ($page_data) {
                foreach ($page_data['data'] as $item) {
                    if ($item['field_type'] == 'file' && $item['is_required'] == 1) {
                        $field_name = $item['input_data'];
                        $existing_count = isset($additional_documents[$field_name]) ? count($additional_documents[$field_name]) : 0;
                        $removed_count = isset($request->removed_additional_documents[$field_name]) ? count($request->removed_additional_documents[$field_name]) : 0;
                        $new_count = isset($request->additional_documents[$field_name]) ? count($request->additional_documents[$field_name]) : 0;

                        if (($existing_count - $removed_count + $new_count) < 1) {
                            $validator->errors()->add($field_name, translate('messages.atleast_one_file_is_required_for_'.$field_name));
                        }
                    }
                }
            }
        ...
        $additional_documents = [];
        if ($delivery_man['additional_documents']) {
            $additional_documents = json_decode($delivery_man['additional_documents'], true);
        }

        if ($request->has('removed_additional_documents')) {
            foreach ($request->removed_additional_documents as $key => $filesToRemove) {
                if (isset($additional_documents[$key])) {
                    foreach ($filesToRemove as $fileToRemove) {
                        $additional_documents[$key] = array_filter($additional_documents[$key], function ($item) use ($fileToRemove) {
                            return ($item['file'] ?? '') !== $fileToRemove;
                        });

                        Helpers::check_and_delete('additional_documents/dm/', $fileToRemove);
                    }
                    $additional_documents[$key] = array_values($additional_documents[$key]);
                }
            }
        }
        ...
        if ($request->additional_documents) {
            foreach ($request->additional_documents as $key => $data) {
                $additional = $additional_documents[$key] ?? [];
                foreach ($data as $file) {
                    if (is_file($file)) {
                        $file_name = Helpers::upload('additional_documents/dm/', 'png', $file);
                        $additional[] = ['file' => $file_name, 'storage' => Helpers::getDisk()];
                    }
                }
                $additional_documents[$key] = $additional;
    ...
    public function status(Request $request)
    {
        ...
        try {
            if ($request->status == 0) {
                ...
                $message = Helpers::getPushNotificationMessage(status: 'deliveryman_account_block', userType: 'deliveryman', lang: $delivery_man->current_language_key, deliveryManName: $delivery_man->f_name.' '.$delivery_man->l_name);
                ...
                $message = Helpers::getPushNotificationMessage(status: 'deliveryman_account_unblock', userType: 'deliveryman', lang: $delivery_man->current_language_key, deliveryManName: $delivery_man->f_name.' '.$delivery_man->l_name);
                if ($message && isset($delivery_man->fcm_token)) {
    ...
    public function dm_list_export(Request $request)
    {
        try {
            ...
            $delivery_men = DeliveryMan::with(['orders', 'rating', 'zone'])
                ...
                ->where('type', 'zone_wise')->latest()->where('application_status', $status ?? 'approved')
                ->when(isset($key), function ($q) use ($key) {
                    $q->where(function ($q) use ($key) {
                        foreach ($key as $value) {
                            $q->orWhere('f_name', 'like', "%{$value}%")
    ...
    {
        $key = explode(' ', $request->get('search', ''));

        $zone_ids = $request->query('zone_id', []);
        $vehicle_ids = $request->query('vehicle_id', []);
        $job_types = $request->query('job_type', []);

        $delivery_men = DeliveryMan::query()
            ->when(! empty($zone_ids) && ! in_array('all', (array) $zone_ids), function ($query) use ($zone_ids) {
                $query->whereIn('zone_id', (array) $zone_ids);
            })
            ...
            ->when(! empty($vehicle_ids) && ! in_array('all', (array) $vehicle_ids), function ($q) use ($vehicle_ids) {
                $q->whereIn('vehicle_id', (array) $vehicle_ids);
            })
            ->when(! empty($job_types) && ! in_array('all', (array) $job_types), function ($q) use ($job_types) {
                $q->where(function ($subQ) use ($job_types) {
                    if (in_array('salary_base', $job_types)) {
                        $subQ->orWhere('earning', 0);
                    }
                    if (in_array('freelance', $job_types)) {
                        $subQ->orWhere('earning', 1);
                    }
                });
            })
            ->with('zone')
        ...
        $zone = (is_array($zone_ids) && count($zone_ids) === 1 && is_numeric($zone_ids[0]))
            ? Zone::find($zone_ids[0])
            : null;
    ...
    public function denied(Request $request)
    {
        ...
        $delivery_men = DeliveryMan::query()
            ...
            ->when(! empty($vehicle_ids) && ! in_array('all', (array) $vehicle_ids), function ($query) use ($vehicle_ids) {
        ...
        //        $zone = is_numeric($zone_id) ? Zone::find($zone_id) : null;

        return view('admin-views.delivery-man.denied', compact('delivery_men'));
    }
