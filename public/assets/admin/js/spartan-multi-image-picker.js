    ...
    $.fn.spartanMultiImagePicker = function (options) {
        ...
        function addRow(settings, el) {
            ...
            var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">` +
                ...
                `${uploadLoaderIcon}` +
                `</div>` +
                `<label class="file_upload" style="width: 100%; height: ${rowHeight}; border: 2px dashed #ddd; border-radius: 3px; cursor: pointer; text-align: center; padding: 5px; margin-top: 5px; margin-bottom : 5px; position : relative; display: flex; align-items: center; margin: auto; justify-content: center; flex-direction: column;">` +
                `<a href="javascript:void(0)" data-spartanindexremove="${count}" style="right: -10px!important;top: -10px!important;background: rgb(237, 60, 32) !important;border-radius: 50px!important;width: 30px !important;height: 30px !important;line-height: 30px !important;text-align: center !important;text-decoration: none !important;color: rgb(255, 255, 255) !important;position: absolute !important; display: none;" class="spartan_remove_row z-index-2"><i class="tio-clear"></i></a>` +
                ...
                `<a href="javascript:void(0)" class="btn btn-outline-info icon-btn view-identity-image" data-spartanindexview="${count}" style="border-radius: 10%; width: 30px; height: 30px; display: none; align-items: center; justify-content: center;"><i class="tio-invisible"></i></a>` +
                `</div>` +
                ...
                if (regex.test(file_select_type) || allowedExt == '') {
                    ...
                    if ((maxFileSize == '') || (maxFileSize != '' && file_select.size <= maxFileSize)) {
                        ...
                        reader.onload = function (e) {
                            $(parent).find('.file_upload').addClass('has-image');
                            $(parent).find('img[data-spartanindexi="' + index + '"]').hide();
                            $(parent).find('a[data-spartanindexremove="' + index + '"]').show();
                            $(parent).find('a[data-spartanindexview="' + index + '"]').css('display', 'flex');
                            $(parent).find('a[data-spartanindexview="' + index + '"]').attr('data-url', e.target.result);
