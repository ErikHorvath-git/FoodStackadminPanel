...
@section('content')
    ...
    <div class="content container-fluid">
                ...
                <div class="flex-grow-1">
                    <div class="d-flex align-items-start">
                        ...
                        <div class="w-0 flex-grow pl-2">
                            <h1 class="page-header-title d-flex gap-2 align-items-center">
                                {{-- {{ $restaurant->name }} {{translate('Subscription')}} &nbsp; &nbsp; --}}
        ...
        <div class="card card-body mb-20">
            <h3 class="text--title card-title mb-20">{{ translate('Filter') }}</h3>
        ...
        <div class="card">
                ...
                <div class="search--button-wrapper justify-content-end">
                    ...
                    <form class="search-form">
                        <div class="input-group input--group">
                            <input name="search" type="search" value="{{ request()?->search }}" class="form-control h--40px" placeholder="{{ translate('messages.Search_by...') }}" aria-label="Search here">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-borderless middle-align __txt-14px text-nowrap">
                        <thead class="thead-light white--space-false">
                            ...
                            <th class="border-top px-4 border-bottom">{{ translate('Transaction_Date') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('Package_Name') }}</th>
                            <th class="border-top px-4 border-bottom text-right">{{ translate('Pricing') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('Duration') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('Payment Method') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-4">
                                    <div>{{ \App\CentralLogics\Helpers::date_format($transaction->created_at) }}</div>
                                </td>
                                <td class="px-4">{{ $transaction?->package?->package_name }}</td>
                                <td class="px-4 text-right">
                                    <div class="text-title">{{ \App\CentralLogics\Helpers::format_currency($transaction->paid_amount) }}</div>
                                </td>
                                <td class="px-4">{{ $transaction->validity }} {{translate('days')}}</td>
                                <td class="px-4">
                                    <div>
                                        ...
                                        @endif
                                        <div class="text-primary font-medium">{{ translate('Paid_by') }}  {{ translate($transaction->payment_method) }}</div>
                                    </div>
                                </td>
                                <td class="px-4">
                                ...
                                <td class="px-4">
                                    <div class="btn--container justify-content-center">
                                        <button class="btn action-btn btn-outline-success printButton" data-url={{ route('vendor.subscriptionackage.invoice',$transaction->id) }} >
