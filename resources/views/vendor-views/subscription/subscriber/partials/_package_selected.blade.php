    ...
    <div>
        ...
        <div class="change-plan-wrapper align-items-center">
                ...
                <div class="__plan-item shift-renew">
                    <div class="inner-div">
                        <div class="text-center">
                            <h3 class="title text-uppercase">{{ translate('commission')  }}</h3>
                            <h2 class="price h-100">{{  $admin_commission }} %</h2>
                        </div>
                    </div>
                </div>
                <div class="plan-seperator-arrow mx-auto">
                    <img src="{{dynamicAsset('public/assets/admin/img/subscription/exchange.svg')}}" alt="{{translate('commission')}}" class="w-100">
                </div>
            @elseif($restaurant_subscription  &&  !in_array($restaurant_model,['commission','none']))
                <div class="__plan-item shift-renew {{ $restaurant_subscription  && $restaurant_subscription?->package_id ==  $package->id ?  'active' : '' }}">
                    <div class="inner-div">
                        <div class="text-center">
                            <h3 class="title text-uppercase">{{ $restaurant_subscription?->package?->package_name  }}</h3>
                            <h2 class="price h-100">{{  \App\CentralLogics\Helpers::format_currency($restaurant_subscription?->package?->price) }}</h2>
            ...
            @if ($restaurant_subscription  &&  $restaurant_subscription?->package_id !==  $package->id || $restaurant_model == 'commission' )
                <div class="__plan-item shift-renew active">
                    <div class="inner-div">
                        <div class="text-center">
                            <h3 class="title text-uppercase">{{$package->package_name }}</h3>
                            <h2 class="price h-100">{{ \App\CentralLogics\Helpers::format_currency($package?->price) }}</h2>
        ...
        <div class="mb-2 mb-lg-3 subscription__plan-info-wrapper bg-light border shadow-sm py-3 rounded-10">
            <div class="row g-3">
                ...
                <div class="col-md-{{ $pending_bill > 0 ? '3' :'4' }}">
                    <div class="subscription__plan-info">
                        ...
                        <h4 class="subtitle">  {{  $restaurant_model != 'commission' &&  $restaurant_subscription?->package_id ==  $package->id ?
                            translate('Renew') : translate('Migrate_to_new_plan') }}
                        </h4>
                    </div>
                </div>
            </div>
                ...
                @if ($balance > 0)
                    <div class="col-md-6">
                        <label class="payment-item">
                            <div  data-toggle="tooltip" data-placement="bottom" data-title="{{$balance >= $package?->price ? translate('pay_the_amount_via_wallet') : translate('You have not sufficient balance on you wallet! please add money to your wallet to purchase the packages') }}">
                                <input type="radio" {{ $balance >= $package?->price ? '' :'disabled'  }} value="wallet"  class="d-none" name="payment_gateway">
                                <div class="payment-item-inner {{ $balance >= $package?->price ? '' : 'border-danger bg-soft-danger' }}">
                                    <div class="check">
                                        <img src="{{ dynamicAsset('/public/assets/admin/img/check-1.png') }}" class="uncheck" alt="">
                                        <img src="{{ dynamicAsset('/public/assets/admin/img/check-3.png') }}" class="check" alt="">
                                    </div>
                                    <span class="d-flex gap-2 align-items-center">
                                        <img width="14" src="{{ dynamicAsset('/public/assets/admin/img/wallet-icon.png') }}" alt="">
                                        {{ translate('wallet') }}
                                    </span>
                                    <span class="ml-auto fs-18 font-bold">
                    ...
                    <div class="col-md-6">
                        <label class="payment-item">
                            ...
                            <div class="payment-item-inner">
                                ...
                                <img class="ml-auto fs-18 font-bold"
