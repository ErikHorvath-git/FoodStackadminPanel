...
@push('script_2')
   ...
   <script>
       (function () {
           const detailsStateKey = "addonActivationOpenCardIndex";
           const scrollStateKey = "addonActivationScrollY";

           function getCards() {
               return Array.from(document.querySelectorAll(".view-details-container"));
           }

           function getOpenCardIndex(cards) {
           ...
           function savePageState() {
               ...
               const openIndex = getOpenCardIndex(cards);
               if (openIndex >= 0) {
                   sessionStorage.setItem(detailsStateKey, String(openIndex));
               }
           ...
           function restorePageState() {
               const cards = getCards();
               const openIndex = Number(sessionStorage.getItem(detailsStateKey));
               const storedScroll = Number(sessionStorage.getItem(scrollStateKey));

               if (!Number.isNaN(openIndex) && cards[openIndex]) {
                   const card = cards[openIndex];
                   const details = card.querySelector(".view-details");
                   const toggleBtn = card.querySelector(".view-btn");
           ...
           document.addEventListener("DOMContentLoaded", function () {
               ...
               document.querySelectorAll(".view-btn").forEach((btn) => {
                   btn.addEventListener("click", function () {
                       setTimeout(savePageState, 350);
                   });
               });

               document.querySelectorAll(".view-details-container form").forEach((form) => {
                   form.addEventListener("submit", savePageState);
               });

               document.addEventListener("click", function (event) {
                   if (event.target.closest(".confirm-Toggle") || event.target.closest(".confirm-Status-Toggle")) {
                       savePageState();
                   }
               });
           });
